...
# python inference_gfpgan_gpu.py -i results/inpt -o results -v 1.4 -s 2 --bg_upsampler realesrgan --bg_tile 128
...
def main():
    """Inference demo for GFPGAN (GPU + alpha-safe)."""
    ...
    if args.bg_upsampler == 'realesrgan':
        ...
        bg_upsampler = RealESRGANer(
            ...
            tile_pad=10,
            pre_pad=0,
            half=use_half,     # FP16 on GPU, False on CPU
            device=device      # ensure it uses CUDA if available
        )
        print(
            f"[GFPGAN] BG upsampler: RealESRGANer on {device} | tile={args.bg_tile}")
    else:
        bg_upsampler = None
